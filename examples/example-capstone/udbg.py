...
def _dbg_trace(mu, address, size, self):
    ...
    return _dbg_trace_internal(mu, address, size, self)  # 调用调试方法
...
def _dbg_trace_internal(mu, address, size, self):
    ...
    while True:
        ...
        try:
            ...
            elif command[0] == 's' or command[0] == 'step':  # 单步
...
class UnicornDebugger:
    def __init__(self, mu, mode=UDBG_MODE_ALL):
        ...
        if mode == UDBG_MODE_ALL:
            mu.hook_add(UC_HOOK_CODE, _dbg_trace, self)  # 指令每次执行都会调用 _dbg_trace 回调

        mu.hook_add(UC_HOOK_MEM_UNMAPPED, _dbg_memory, self)  # 异常hook
        mu.hook_add(UC_HOOK_MEM_FETCH_PROT, _dbg_memory, self)  # 异常hook
